    ...
    "graph": {
      "blocks": [
        ...
        },
        {
          ...
          "type": "basic.code",
          "data": {
            "code": "// Parallel interface\nreg [2:0] rs_latch=0;\nreg [7:0] d_latch=0, c_latch = 8'h70;\nreg [11:0] waddr = 0;\nreg [15:0] d_out=0;\nreg we=0;\nreg [3:0] clockfilter=0;\nreg request=0, ready = 0;\n\nalways @ (posedge i_clock)\nbegin\n    clockfilter <= {clockfilter[2:0],e_clock};\n    request <= !cs && !wr && (clockfilter==4'b1100);\n    rs_latch <= rs;\n    d_latch <= d;\nend\n\nwire write_out = request & (rs_latch==3'b000);\nwire [11:0] next_addr = ((waddr == 2399)||(rs_latch==3'h4))?0:waddr <= waddr + 1;\nwire set_waddr = request & (rs_latch[1:0] == 2'b00);\nwire write_c = request & (rs_latch==3'b001);\n\nalways @ (posedge i_clock)\nbegin\n    d_out <= {c_latch, d_latch};    // color & data\n    we <= write_out;\n    if (set_waddr) waddr <= next_addr;\n    if (write_c) c_latch <= d_latch;\nend\n",
            "params": [],
            "ports": {
              "in": [
                {
